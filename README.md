...
- 目录大小：该叶子目录直属文件的大小总和。
- 硬链接：`--all` 目录总量按同一 NTFS 文件记录（FRN）去重统计，避免同一份磁盘簇因多个硬链接路径被重复计入；文件列表仍会展示可见的硬链接路径。DOS-only 8.3 别名不作为额外目录项统计。
- 资源管理器口径差异：`--all` 目录总量更接近“唯一文件实体”或卷级物理占用口径；大量硬链接路径仍会在文件列表中可见，但不会把同一份分配空间重复加入目录总量。
